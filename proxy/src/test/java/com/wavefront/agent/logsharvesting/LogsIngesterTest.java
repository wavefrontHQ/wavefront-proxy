...
public class LogsIngesterTest {
  ...
  private LogsIngestionConfig parseConfigFile(String configPath) throws IOException {
    File configFile =
        new File(LogsIngesterTest.class.getClassLoader().getResource(configPath).getPath());
    return objectMapper.readValue(configFile, LogsIngestionConfig.class);
  }

  private void setup(LogsIngestionConfig config)
      throws IOException, GrokException, ConfigurationException {
  ...
  private List<ReportPoint> getPoints(
      int numPoints, int lagPerLogLine, Consumer<String> consumer, String... logLines)
      throws Exception {
  ...
  public void testPrefixIsApplied() throws Exception {
    ...
    logsIngesterUnderTest =
        new LogsIngester(mockFactory, () -> logsIngestionConfig, "myPrefix", now::get, nanos::get);
    assertThat(
        getPoints(1, "plainCounter"),
        contains(
            PointMatchers.matches(
                1L,
                MetricConstants.DELTA_PREFIX + "myPrefix" + ".plainCounter",
                ImmutableMap.of())));
  }
  ...
  public void testFilebeatIngesterDefaultHostname() throws Exception {
    ...
    assertThat(
        ...
        contains(
            PointMatchers.matches(
                1L,
                MetricConstants.DELTA_PREFIX + "plainCounter",
                "parsed-logs",
                ImmutableMap.of())));
  }
  ...
  public void testEmptyTagValuesIgnored() throws Exception {
    ...
    assertThat(
        getPoints(
            ...
            0,
            this::receiveRawLog,
            "pingSSO|2020-03-13 09:11:30,490|OAuth| RLin123456| "
        ...
        contains(
            PointMatchers.matches(
                ...
                ImmutableMap.<String, String>builder()
                    .put("sso_host", "sso2-prod")
                    .put("protocol", "OAuth20")
                    .put("role", "AS")
                    .put("subject", "RLin123456")
                    .put("ip", "10.0.0.1")
                    .put("connectionid", "pa_wam")
                    .put("adapterid", "SecurID")
                    .put("event", "OAuth")
                    .put("status", "success")
                    .build())));
  }
  ...
  public void testHotloadedConfigClearsOldMetrics() throws Exception {
    ...
    assertThat(
        ...
        containsInAnyOrder(
            ImmutableList.of(
                PointMatchers.matches(
                    42L, MetricConstants.DELTA_PREFIX + "counterWithValue", ImmutableMap.of()),
                PointMatchers.matches(
                    1L, MetricConstants.DELTA_PREFIX + "plainCounter", ImmutableMap.of()))));
    ...
    // Get a new config file because the SUT has a reference to the old one, and we'll be monkey
    // patching
    // this one.
  ...
  public void testEvictedDeltaMetricReportingAgain() throws Exception {
    ...
    assertThat(
        ...
        contains(
            PointMatchers.matches(
                4L, MetricConstants.DELTA_PREFIX + "plainCounter", ImmutableMap.of())));
    nanos.addAndGet(1_800_000L * 1000L * 1000L);
    assertThat(
        getPoints(1, "plainCounter", "plainCounter", "plainCounter"),
        contains(
            PointMatchers.matches(
                3L, MetricConstants.DELTA_PREFIX + "plainCounter", ImmutableMap.of())));
    nanos.addAndGet(3_601_000L * 1000L * 1000L);
    assertThat(
        getPoints(1, "plainCounter", "plainCounter"),
        contains(
            PointMatchers.matches(
                2L, MetricConstants.DELTA_PREFIX + "plainCounter", ImmutableMap.of())));
  }
  ...
  public void testMetricsAggregation() throws Exception {
    ...
    assertThat(
        getPoints(
            ...
            "counterWithValue 2",
            "counterWithValue 3",
            "dynamicCounter foo 1 done",
            "dynamicCounter foo 2 done",
            "dynamicCounter baz 1 done"),
        containsInAnyOrder(
            ImmutableList.of(
                PointMatchers.matches(
                    2L, MetricConstants.DELTA_PREFIX + "plainCounter", ImmutableMap.of()),
                PointMatchers.matches(
                    5L, MetricConstants.DELTA_PREFIX + "counterWithValue", ImmutableMap.of()),
                PointMatchers.matches(
                    1L, MetricConstants.DELTA_PREFIX + "dynamic_foo_1", ImmutableMap.of()),
                PointMatchers.matches(
                    1L, MetricConstants.DELTA_PREFIX + "dynamic_foo_2", ImmutableMap.of()),
                PointMatchers.matches(
                    1L, MetricConstants.DELTA_PREFIX + "dynamic_baz_1", ImmutableMap.of()),
    ...
    assertThat(
        ...
        containsInAnyOrder(
            ImmutableList.of(
                PointMatchers.matches(
                    4L,
                    MetricConstants.DELTA_PREFIX + "Host.foo.totalSeconds",
                    "web001.acme.corp",
                    ImmutableMap.of("static", "value")),
                PointMatchers.matches(
                    3L,
                    MetricConstants.DELTA_PREFIX + "Host.foo.totalSeconds",
                    "web002.acme.corp",
                ...
                PointMatchers.matches(
                    4L,
                    MetricConstants.DELTA_PREFIX + "Host.bar.totalSeconds",
                    "web001.acme.corp",
                    ImmutableMap.of("static", "value")))));
  }

  /**
   * This test is not required, because delta counters have different naming convention than
   * gauges @Test(expected = ClassCastException.class) public void testDuplicateMetric() throws
   * Exception { setup(parseConfigFile("dupe.yml")); assertThat(getPoints(2, "plainCounter",
   * "plainGauge 42"), notNullValue()); }
   */
  @Test
    ...
    assertThat(
        ...
        containsInAnyOrder(
            ImmutableList.of(
                PointMatchers.matches(
                    4L,
                    MetricConstants.DELTA_PREFIX + "foo.totalSeconds",
                ...
                PointMatchers.matches(
                    4L,
                    MetricConstants.DELTA_PREFIX + "bar.totalSeconds",
                    ImmutableMap.of("theDC", "wavefront", "theAZ", "2a")))));
  }
    ...
    assertThat(
        ...
        containsInAnyOrder(
            ImmutableList.of(
                PointMatchers.matches(
                    4L,
                    MetricConstants.DELTA_PREFIX + "TagValue.foo.totalSeconds",
                    ImmutableMap.of(
                        "theDC",
                        "wavefront",
                        "theAZ",
                ...
                PointMatchers.matches(
                    4L,
                    MetricConstants.DELTA_PREFIX + "TagValue.bar.totalSeconds",
                    ImmutableMap.of(
                        "theDC",
                        "wavefront",
                        "theAZ",
  ...
  public void testAdditionalPatterns() throws Exception {
    ...
    assertThat(
        ...
        contains(
            PointMatchers.matches(
                42L, MetricConstants.DELTA_PREFIX + "customPatternCounter", ImmutableMap.of())));
  }
  ...
  public void testParseValueFromCombinedApacheLog() throws Exception {
    ...
    assertThat(
        ...
        containsInAnyOrder(
            ImmutableList.of(
                PointMatchers.matches(
                    632L, MetricConstants.DELTA_PREFIX + "apacheBytes", ImmutableMap.of()),
                PointMatchers.matches(
                    632L, MetricConstants.DELTA_PREFIX + "apacheBytes2", ImmutableMap.of()),
  ...
  public void testWavefrontHistogram() throws Exception {
    ...
    ReportPoint reportPoint =
        getPoints(mockHistogramHandler, 1, 0, this::receiveLog, logs.toArray(new String[0])).get(0);
    ...
    assertThat(
        wavefrontHistogram.getBins().get(wavefrontHistogram.getBins().size() - 1), equalTo(100.0));
  }
  ...
  public void testWavefrontHistogramMultipleCentroids() throws Exception {
    ...
    List<ReportPoint> reportPoints =
        getPoints(mockHistogramHandler, 2, 500, this::receiveLog, lines);
    assertThat(reportPoints.size(), equalTo(2));
    assertThat(
        reportPoints,
        containsInAnyOrder(
            PointMatchers.histogramMatches(120, 7260.0),
