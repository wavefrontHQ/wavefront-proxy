...
public class QueuedAgentServiceTest {
    ...
    public void postWorkUnitResultCallsApproriateServiceMethodAndReturnsOK() {
        ...
        EasyMock
            .expect(mockAgentAPI.postWorkUnitResult(agentId, workUnitId, targetId, shellOutputDTO)).
            andReturn(Response.ok().build()).once();
        EasyMock.replay(mockAgentAPI);

        Response response =
            queuedAgentService.postWorkUnitResult(agentId, workUnitId, targetId, shellOutputDTO);
    ...
    public void postPushDataCallsApproriateServiceMethodAndReturnsOK() {
        ...
        EasyMock
            .expect(mockAgentAPI.postPushData(agentId, workUnitId, now, format, pretendPushData)).
            andReturn(Response.ok().build()).once();
        EasyMock.replay(mockAgentAPI);

        Response response =
            queuedAgentService.postPushData(agentId, workUnitId, now, format, pretendPushData);
    ...
    public void postWorkUnitResultTaskExecuteCallsAppropriateService() {
        ...
        QueuedAgentService.PostWorkUnitResultTask task =
            new QueuedAgentService.PostWorkUnitResultTask(agentId, workUnitId, targetId,
                shellOutputDTO);

        injectServiceToResubmissionTask(task);

        EasyMock.expect(
            mockAgentAPI.postWorkUnitResult(newAgentId, workUnitId, targetId, shellOutputDTO))
            .andReturn(Response.ok().build()).once();
    ...
    public void postPushDataResultTaskExecuteCallsAppropriateService() {
        ...
        QueuedAgentService.PostPushDataResultTask task =
            new QueuedAgentService.PostPushDataResultTask(agentId, workUnitId, now, format,
                pretendPushData);

        injectServiceToResubmissionTask(task);

        EasyMock
            .expect(mockAgentAPI.postPushData(newAgentId, workUnitId, now, format, pretendPushData))
            .andReturn(Response.ok().build()).once();
    ...
    public void postPushDataResultTaskSplitTaskSplitsEvenly() {
        ...
        PostPushDataResultTask task =
            new PostPushDataResultTask(agentId, workUnitId, now, format, pretendPushData);
            ...
            for (int numTestStrings = 1; numTestStrings <= 51; numTestStrings += 1) {
                ...
                for (PostPushDataResultTask taskN : splitTasks) {
                    List<String> dataStrings =
                        ChannelStringHandler.unjoinPushData(taskN.getPushData());
