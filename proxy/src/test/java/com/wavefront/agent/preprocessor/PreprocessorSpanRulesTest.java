...
public class PreprocessorSpanRulesTest {
  ...
  public void testSpanWhitelistAnnotation() {
    String spanLine =
        "\"testSpanName\" \"source\"=\"spanSourceName\" "
  ...
  public void testSpanLimitRuleDropSpanNameThrows() {
    new SpanLimitLengthTransformer(
        SPAN_NAME, 10, LengthLimitActionType.DROP, null, false, null, metrics);
  }

  @Test(expected = IllegalArgumentException.class)
  public void testSpanLimitRuleDropSourceNameThrows() {
    new SpanLimitLengthTransformer(
        SOURCE_NAME, 10, LengthLimitActionType.DROP, null, false, null, metrics);
  }

  @Test(expected = IllegalArgumentException.class)
  public void testSpanLimitRuleTruncateWithEllipsisMaxLengthLessThan3Throws() {
    new SpanLimitLengthTransformer(
        "parent", 1, LengthLimitActionType.TRUNCATE_WITH_ELLIPSIS, null, false, null, metrics);
  }

  @Test
  public void testSpanFiltersWithValidV2AndInvalidV1Predicate() {
    try {
      SpanAllowFilter invalidRule = new SpanAllowFilter("spanName", null, x -> false, metrics);
    } catch (IllegalArgumentException e) {
      // Expected.
    }

    try {
      SpanAllowFilter invalidRule = new SpanAllowFilter(null, "^host$", x -> false, metrics);
    } catch (IllegalArgumentException e) {
      // Expected.
    }

    try {
      SpanAllowFilter invalidRule = new SpanAllowFilter("spanName", "^host$", x -> false, metrics);
    } catch (IllegalArgumentException e) {
      // Expected.
    }

    SpanAllowFilter validWhitelistRule = new SpanAllowFilter(null, null, x -> false, metrics);

    try {
      SpanBlockFilter invalidRule = new SpanBlockFilter("metricName", null, x -> false, metrics);
    } catch (IllegalArgumentException e) {
      // Expected.
    }

    try {
      SpanBlockFilter invalidRule = new SpanBlockFilter(null, "^host$", x -> false, metrics);
    } catch (IllegalArgumentException e) {
      // Expected.
    }

    try {
      SpanBlockFilter invalidRule = new SpanBlockFilter("spanName", "^host$", x -> false, metrics);
    } catch (IllegalArgumentException e) {
      // Expected.
    }

    SpanBlockFilter validBlockRule = new SpanBlockFilter(null, null, x -> false, metrics);
  }

  @Test
  public void testSpanFiltersWithValidV2AndV1Predicate() {
    SpanAllowFilter validAllowRule = new SpanAllowFilter(null, null, x -> false, metrics);
  ...
  public void testSpanLimitRule() {
    ...
    rule =
        new SpanLimitLengthTransformer(
            SPAN_NAME, 8, LengthLimitActionType.TRUNCATE, null, false, null, metrics);
    ...
    rule =
        new SpanLimitLengthTransformer(
            SPAN_NAME, 8, LengthLimitActionType.TRUNCATE, "nope.*", false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    ...
    rule =
        new SpanLimitLengthTransformer(
            SOURCE_NAME, 10, LengthLimitActionType.TRUNCATE, null, false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    ...
    rule =
        new SpanLimitLengthTransformer(
            SOURCE_NAME, 10, LengthLimitActionType.TRUNCATE, "nope.*", false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    ...
    rule =
        new SpanLimitLengthTransformer(
            FOO, 4, LengthLimitActionType.TRUNCATE, null, false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar1", "bar2", "bar2", "baz"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
    ...
    rule =
        new SpanLimitLengthTransformer(
            FOO, 4, LengthLimitActionType.DROP, null, false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
    ...
    rule =
        new SpanLimitLengthTransformer(
            FOO, 4, LengthLimitActionType.TRUNCATE_WITH_ELLIPSIS, "bar2-.*", false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar1-1234567890", "b...", "b...", "baz"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    rule =
        new SpanLimitLengthTransformer(
            FOO, 4, LengthLimitActionType.TRUNCATE, "bar2-.*", true, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar1-1234567890", "bar2", "bar2-2345678901", "baz"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    rule =
        new SpanLimitLengthTransformer(
            FOO, 4, LengthLimitActionType.DROP, "bar2-.*", true, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar1-1234567890", "bar2-2345678901", "baz"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    rule =
        new SpanLimitLengthTransformer(
            FOO, 4, LengthLimitActionType.TRUNCATE, ".*nope.*", false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar1-1234567890", "bar2-1234567890", "bar2-2345678901", "baz"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
  ...
  public void testSpanAddAnnotationRule() {
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar1-1234567890", "bar2-1234567890", "bar2-2345678901", "baz", "baz2"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
  ...
  public void testSpanDropAnnotationRule() {
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar2-1234567890", "bar2-2345678901", "baz"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar1-1234567890", "baz"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
  ...
  public void testSpanExtractAnnotationRule() {
    ...
    rule =
        new SpanExtractAnnotationTransformer(
            "boo", FOO, "(....)-(.*)$", "$2", "$1", null, true, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("baz", "1234567890"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals("boo"))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    assertEquals(
        ImmutableList.of("bar1", "bar2-2345678901", "bar2-3456789012", "bar"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    rule =
        new SpanExtractAnnotationTransformer(
            "boo", FOO, "(....)-(.*)$", "$2", "$1", "bar2.*", true, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("baz", "2345678901"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals("boo"))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    assertEquals(
        ImmutableList.of("bar1-1234567890", "bar2", "bar2-3456789012", "bar"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    rule =
        new SpanExtractAnnotationTransformer(
            "boo", FOO, "(....)-(.*)$", "$2", "$1", null, false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("baz", "1234567890", "2345678901", "3456789012"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals("boo"))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    assertEquals(
        ImmutableList.of("bar1", "bar2", "bar2", "bar"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
  ...
  public void testSpanExtractAnnotationIfNotExistsRule() {
    ...
    rule =
        new SpanExtractAnnotationIfNotExistsTransformer(
            "baz", FOO, "(....)-(.*)$", "$2", "$1", null, true, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("1234567890"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals("baz"))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    assertEquals(
        ImmutableList.of("bar1", "bar2-2345678901", "bar2-3456789012", "bar"),
    ...
    rule =
        new SpanExtractAnnotationIfNotExistsTransformer(
            "baz", FOO, "(....)-(.*)$", "$2", "$1", "bar2.*", true, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("2345678901"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals("baz"))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    rule =
        new SpanExtractAnnotationIfNotExistsTransformer(
            "baz", FOO, "(....)-(.*)$", "$2", "$1", null, false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("1234567890", "2345678901", "3456789012"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals("baz"))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    assertEquals(
        ...
        span.getAnnotations().stream()
            ...
            .collect(Collectors.toList()));
    assertEquals(
        ImmutableList.of("bar1-1234567890", "bar2-2345678901", "bar2-3456789012", "bar"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
  ...
  public void testSpanRenameTagRule() {
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("foo1", "foo1", "foo1", "foo1", "boo"),
        span.getAnnotations().stream().map(Annotation::getKey).collect(Collectors.toList()));
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of(FOO, "foo1", "foo1", FOO, "boo"),
        span.getAnnotations().stream().map(Annotation::getKey).collect(Collectors.toList()));
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of(FOO, "foo1", FOO, FOO, "boo"),
        span.getAnnotations().stream().map(Annotation::getKey).collect(Collectors.toList()));
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of(FOO, FOO, FOO, FOO, "boo"),
        span.getAnnotations().stream().map(Annotation::getKey).collect(Collectors.toList()));
  }
  ...
  public void testSpanReplaceRegexRule() {
    ...
    rule =
        new SpanReplaceRegexTransformer(SOURCE_NAME, "Name", "Z", null, null, false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals("spanSourceZ", span.getSource());

    rule =
        new SpanReplaceRegexTransformer(
            SOURCE_NAME, "Name", "Z", "span.*", null, false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals("spanSourceZ", span.getSource());

    rule =
        new SpanReplaceRegexTransformer(
            SOURCE_NAME, "Name", "Z", "no_match", null, false, null, metrics);
    span = rule.apply(parseSpan(spanLine));
    assertEquals("spanSourceName", span.getSource());
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar1-1zzz567890", "bar2-zzz5678901", "bar2-3456789012", "bar"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar1-1234567890", "bar2-2345678zzz", "bar2-3456789012", "bar"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar@234567890", "bar@345678901", "bar@456789012", "bar"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("bar@234567890", "bar2-2345678901", "bar2-3456789012", "bar"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals(FOO))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        ImmutableList.of("https://localhost:50051/style/foo/make?id=REDACTED"),
        span.getAnnotations().stream()
    ...
    span = rule.apply(parseSpan(spanLine));
    assertEquals(
        "bar1-1234567890-testSpanName-spanSourceName{{}}",
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals("boo"))
            .map(Annotation::getValue)
            .findFirst()
  ...
  public void testSpanSanitizeTransformer() {
    ...
    assertEquals(
        ImmutableList.of("''"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals("special-tag-"))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
    assertEquals(
        ImmutableList.of("hello \\n world"),
        span.getAnnotations().stream()
            .filter(x -> x.getKey().equals("specialvalue"))
            .map(Annotation::getValue)
            .collect(Collectors.toList()));
  }
}
