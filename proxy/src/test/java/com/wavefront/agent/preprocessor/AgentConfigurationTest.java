...
public class AgentConfigurationTest {
  ...
  public void testLoadInvalidRules() {
    ...
    try {
      InputStream stream =
          PreprocessorRulesTest.class.getResourceAsStream("preprocessor_rules_invalid.yaml");
      config.loadFromStream(stream);
      fail("Invalid rules did not cause an exception");
    } catch (RuntimeException ex) {
      Assert.assertEquals(1, config.totalValidRules);
      Assert.assertEquals(137, config.totalInvalidRules);
    }
  }
  ...
  public void testLoadValidRules() {
    ...
    Assert.assertEquals(63, config.totalValidRules);
  }
  ...
  public void testPreprocessorRulesOrder() {
    ...
    InputStream stream =
        PreprocessorRulesTest.class.getResourceAsStream("preprocessor_rules_order_test.yaml");
    PreprocessorConfigManager config = new PreprocessorConfigManager();
    config.loadFromStream(stream);
    config
  ...
  public void testMultiPortPreprocessorRules() {
    ...
    InputStream stream =
        PreprocessorRulesTest.class.getResourceAsStream("preprocessor_rules_multiport.yaml");
    PreprocessorConfigManager config = new PreprocessorConfigManager();
    config.loadFromStream(stream);
    ReportPoint point =
        new ReportPoint(
