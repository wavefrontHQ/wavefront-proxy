...
public class PreprocessorLogRulesTest {
  ...
  public static void setup() throws IOException {
    InputStream stream = PreprocessorRulesTest.class.getResourceAsStream("log_preprocessor_rules" + ".yaml");
  ...
  public void testReportLogAddTagIfNotExistTransformer() {
    // Arrange
    ReportLog log = ReportLog.newBuilder().setTimestamp(0).setHost("").setMessage("").setAnnotations(new ArrayList<>()).build();
    ReportLogAddTagIfNotExistsTransformer rule = new ReportLogAddTagIfNotExistsTransformer("foo", "bar", null, metrics);
  ...
  public void testReportLogAddTagTransformer() {
    ...
    ReportLog log = ReportLog.newBuilder().setTimestamp(0).setHost("").setMessage("").setAnnotations(existingAnnotations).build();
  ...
  public void testReportLogAllowFilters_testAllowed() {
    ...
    List<Annotation> existingAnnotations = new ArrayList<>();
    existingAnnotations.add(Annotation.newBuilder().setKey("foo").setValue("bar").build());
    ReportLog log = ReportLog.newBuilder().setTimestamp(0).setHost("abc").setMessage("123").setAnnotations(existingAnnotations).build();

    ReportLogAllowFilter filter1 = new ReportLogAllowFilter("message", "^[0-9]+", x -> true, metrics);
    ReportLogAllowFilter filter2 = new ReportLogAllowFilter("sourceName", "^[a-z]+", x -> true, metrics);
  ...
  public void testReportLogAllowTagTransformer() {
    ...
    ReportLogAllowTagTransformer rule = new ReportLogAllowTagTransformer(allowedTags, null, metrics);
  ...
  public void testReportLogBlockFilters_testNotAllowed() {
    ...
    ReportLogBlockFilter filter1 = new ReportLogBlockFilter("message", "^[0-9]+", x -> true, metrics);
    ReportLogBlockFilter filter2 = new ReportLogBlockFilter("sourceName", "^[a-z]+", x -> true, metrics);
  ...
  public void testReportLogDropTagTransformer() {
    ...
    Map<String, String> allowedTags = new HashMap<>();
    allowedTags.put("k1", "v1");
    ReportLogDropTagTransformer rule = new ReportLogDropTagTransformer("foo", "bar1", null, metrics);
  ...
  public void testReportLogExtractTagTransformer() {
    ...
    List<Annotation> existingAnnotations = new ArrayList<>();
    existingAnnotations.add(Annotation.newBuilder().setKey("foo").setValue("bar").build());
    ReportLog log = ReportLog.newBuilder().setTimestamp(0).setHost("abc").setMessage("123abc123").setAnnotations(existingAnnotations).build();

    ReportLogExtractTagTransformer messageRule = new ReportLogExtractTagTransformer("t1", "message", "([0-9]+)([a-z]+)([0-9]+)", "$2", "$1$3", "([0-9a-z]+)", null, metrics);

    ReportLogExtractTagTransformer sourceRule = new ReportLogExtractTagTransformer("t2", "sourceName", "(a)(b)(c)", "$2$3", "$1$3", "^[a-z]+", null, metrics);

    ReportLogExtractTagTransformer annotationRule = new ReportLogExtractTagTransformer("t3", "foo", "(b)(ar)", "$1", "$2", "^[a-z]+", null, metrics);

    ReportLogExtractTagTransformer invalidPatternMatch = new ReportLogExtractTagTransformer("t3", "foo", "asdf", "$1", "$2", "badpattern", null, metrics);

    ReportLogExtractTagTransformer invalidPatternSearch = new ReportLogExtractTagTransformer("t3", "foo", "badpattern", "$1", "$2", "^[a-z]+", null, metrics);
  ...
  public void testReportLogForceLowerCaseTransformer() {
    ...
    List<Annotation> existingAnnotations = new ArrayList<>();
    existingAnnotations.add(Annotation.newBuilder().setKey("foo").setValue("baR").build());
    ReportLog log = ReportLog.newBuilder().setTimestamp(0).setHost("Abc").setMessage("dEf").setAnnotations(existingAnnotations).build();

    ReportLogForceLowercaseTransformer messageRule = new ReportLogForceLowercaseTransformer("message", ".*", null, metrics);

    ReportLogForceLowercaseTransformer sourceRule = new ReportLogForceLowercaseTransformer("sourceName", ".*", null, metrics);

    ReportLogForceLowercaseTransformer annotationRule = new ReportLogForceLowercaseTransformer("foo", ".*", null, metrics);

    ReportLogForceLowercaseTransformer messagePatternMismatchRule = new ReportLogForceLowercaseTransformer("message", "doesNotMatch", null, metrics);

    ReportLogForceLowercaseTransformer sourcePatternMismatchRule = new ReportLogForceLowercaseTransformer("sourceName", "doesNotMatch", null, metrics);
  ...
  public void testReportLogLimitLengthTransformer() {
    ...
    List<Annotation> existingAnnotations = new ArrayList<>();
    existingAnnotations.add(Annotation.newBuilder().setKey("foo").setValue("bar").build());
    ReportLog log = ReportLog.newBuilder().setTimestamp(0).setHost("abc").setMessage("123456").setAnnotations(existingAnnotations).build();

    ReportLogLimitLengthTransformer messageRule = new ReportLogLimitLengthTransformer("message", 5, TRUNCATE_WITH_ELLIPSIS, null, null, metrics);

    ReportLogLimitLengthTransformer sourceRule = new ReportLogLimitLengthTransformer("sourceName", 2, TRUNCATE, null, null, metrics);

    ReportLogLimitLengthTransformer annotationRule = new ReportLogLimitLengthTransformer("foo", 2, TRUNCATE, ".*", null, metrics);

    ReportLogLimitLengthTransformer messagePatternMismatchRule = new ReportLogLimitLengthTransformer("message", 2, TRUNCATE, "doesNotMatch", null, metrics);

    ReportLogLimitLengthTransformer sourcePatternMismatchRule = new ReportLogLimitLengthTransformer("sourceName", 2, TRUNCATE, "doesNotMatch", null, metrics);
  ...
  public void testReportLogRenameTagTransformer() {
    ...
    ReportLogRenameTagTransformer annotationRule = new ReportLogRenameTagTransformer("foo", "foo2", ".*", null, metrics);

    ReportLogRenameTagTransformer PatternMismatchRule = new ReportLogRenameTagTransformer("foo", "foo3", "doesNotMatch", null, metrics);
  ...
  public void testReportLogReplaceRegexTransformer() {
    ...
    ReportLogReplaceRegexTransformer messageRule = new ReportLogReplaceRegexTransformer("message", "12", "ab", null, 5, null, metrics);

    ReportLogReplaceRegexTransformer sourceRule = new ReportLogReplaceRegexTransformer("sourceName", "ab", "12", null, 5, null, metrics);

    ReportLogReplaceRegexTransformer annotationRule = new ReportLogReplaceRegexTransformer("foo", "bar", "ouch", ".*", 5, null, metrics);

    ReportLogReplaceRegexTransformer messagePatternMismatchRule = new ReportLogReplaceRegexTransformer("message", "123", "abc", "doesNotMatch", 5, null, metrics);

    ReportLogReplaceRegexTransformer sourcePatternMismatchRule = new ReportLogReplaceRegexTransformer("sourceName", "abc", "123", "doesNotMatch", 5, null, metrics);
