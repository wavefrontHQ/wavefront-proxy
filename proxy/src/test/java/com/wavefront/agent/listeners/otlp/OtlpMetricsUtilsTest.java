...
public class OtlpMetricsUtilsTest {
    ...
    public void transformsGaugeTimestampToEpochMilliseconds() {
        ...
        Gauge otlpGauge = Gauge.newBuilder()
    ...
    public void handlesGaugeAttributes() {
        KeyValue booleanAttr = KeyValue.newBuilder()
                .setKey("a-boolean")
    ...
    public void createsMetricsForEachSummaryQuantile() {
        Metric otlpMetric = OtlpTestHelpers.otlpSummaryGenerator(
                ImmutableList.of(
                        SummaryDataPoint.ValueAtQuantile.newBuilder()
                                .setQuantile(.2)
                                .setValue(2.2)
    ...
    public void transformsCumulativeHistogramWithoutBounds() {
        HistogramDataPoint point = HistogramDataPoint.newBuilder().addAllBucketCounts(ImmutableList.of(1L)).build();
        Histogram histo = Histogram.newBuilder()
                .setAggregationTemporality(AggregationTemporality.AGGREGATION_TEMPORALITY_CUMULATIVE)
                .addAllDataPoints(Collections.singletonList(point))
                .build();
    ...
    public void transformExpDeltaHistogram() {
        ...
        ExponentialHistogram histo = ExponentialHistogram.newBuilder()
                ...
                .addDataPoints(point)
                .build();
        Metric otlpMetric = OtlpTestHelpers.otlpMetricGenerator().setExponentialHistogram(histo).build();
    ...
    public void setsSource() {
        Metric otlpMetric = OtlpTestHelpers.otlpGaugeGenerator(NumberDataPoint.newBuilder().build()).build();
    ...
    public void appliesPreprocessorRules() {
        List<NumberDataPoint> dataPoints = Collections
                .singletonList(NumberDataPoint.newBuilder().setTimeUnixNano(0).build());
    ...
    public void testAppTagsFromResourceAttrs() {
        Resource otelResource = Resource.newBuilder()
        ...
        List<KeyValue> attrList = OtlpMetricsUtils.appTagsFromResourceAttrs(otelResource.getAttributesList());
        assertEquals(
                "some-app-name",
