...
public class SQSSubmissionQueueTest<T extends DataSubmissionTask<T>> {
  ...
  public SQSSubmissionQueueTest(
      TaskConverter.CompressionType compressionType, RetryTaskConverter<T> converter, T task) {
    ...
    expect(
            client.sendMessage(
                new SendMessageRequest(
                    queueUrl, queue.encodeMessageForDelivery(this.expectedTask))))
        .andReturn(null);
    replay(client);
    task.enqueue(QueueingReason.RETRY);

    reset(client);
    ReceiveMessageRequest msgRequest =
        new ReceiveMessageRequest()
            .withMaxNumberOfMessages(1)
            .withWaitTimeSeconds(1)
            .withQueueUrl(queueUrl);
    ReceiveMessageResult msgResult =
        new ReceiveMessageResult()
            .withMessages(
                new Message()
                    .withBody(queue.encodeMessageForDelivery(task))
