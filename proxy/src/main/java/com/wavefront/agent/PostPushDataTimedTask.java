...
public class PostPushDataTimedTask implements Runnable {
    ...
    private static final Logger logger =
        Logger.getLogger(PostPushDataTimedTask.class.getCanonicalName());
    ...
    public PostPushDataTimedTask(ForceQueueEnabledAgentAPI agentAPI, String logLevel, UUID daemonId,
        int port) {
        ...
        this.pointsAttempted =
            Metrics.newCounter(new MetricName("points." + String.valueOf(port), "", "sent"));
        this.pointsQueued =
            Metrics.newCounter(new MetricName("points." + String.valueOf(port), "", "queued"));
        this.pointsBlocked =
            Metrics.newCounter(new MetricName("points." + String.valueOf(port), "", "blocked"));
        this.pointsReceived =
            Metrics.newCounter(new MetricName("points." + String.valueOf(port), "", "received"));
        this.batchSendTime = Metrics
            .newTimer(new MetricName("push." + String.valueOf(port), "", "duration"),
            ...
            if (current.size() != 0) {
                ...
                if (points.size() > getQueuedPointLimit()) {
                    if (warningMessageRateLimiter.tryAcquire()) {
                        logger.warning(
                            "WF-3 Too many pending points (" + points.size() + "), block size: " +
    ...
    private List<String> createAgentPostBatch() {
        ...
        if (logLevel.equals(LOG_DETAILED)) {
            logger.warning(
                "[" + port + "] (DETAILED): sending " + current.size() + " valid points; " +
