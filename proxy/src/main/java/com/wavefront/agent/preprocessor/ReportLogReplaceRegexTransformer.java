...
public class ReportLogReplaceRegexTransformer implements Function<ReportLog, ReportLog> {
  ...
  public ReportLogReplaceRegexTransformer(final String scope,
  ...
  public ReportLog apply(@Nullable ReportLog reportLog) {
    ...
    try {
      ...
      switch (scope) {
        case "message":
          if (compiledMatchPattern != null && !compiledMatchPattern.matcher(reportLog.getMessage()).matches()) {
            break;
          }
          reportLog.setMessage(replaceString(reportLog, reportLog.getMessage()));
          break;
        case "sourceName":
          if (compiledMatchPattern != null && !compiledMatchPattern.matcher(reportLog.getHost()).matches()) {
            break;
          }
        ...
        default:
          for (Annotation tagKV : reportLog.getAnnotations()) {
            if (tagKV.getKey().equals(scope) && (compiledMatchPattern == null ||
