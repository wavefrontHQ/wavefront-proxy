...
/**
 * Force lowercase transformer. Converts a specified component of a log (message,
 * source name or a log tag value, depending on "scope" parameter) to lower case to
 * enforce consistency.
 *
...
public class ReportLogForceLowercaseTransformer implements Function<ReportLog, ReportLog> {
  ...
  public ReportLogForceLowercaseTransformer(final String scope,
      ...
      switch (scope) {
        ...
        default:
          for (Annotation logTagKV : reportLog.getAnnotations()) {
            if (logTagKV.getKey().equals(scope) && (compiledMatchPattern == null ||
