...
public class SpanDropAnnotationTransformer implements Function<Span, Span> {
  ...
  public SpanDropAnnotationTransformer(
      final String key,
      @Nullable final String patternMatch,
      final boolean firstMatchOnly,
      @Nullable final Predicate<Span> v2Predicate,
      final PreprocessorRuleMetrics ruleMetrics) {
    this.compiledKeyPattern =
        Pattern.compile(Preconditions.checkNotNull(key, "[key] can't be null"));
  ...
  public Span apply(@Nullable Span span) {
    ...
    try {
      ...
      while (iterator.hasNext()) {
        ...
        if (compiledKeyPattern.matcher(entry.getKey()).matches()
