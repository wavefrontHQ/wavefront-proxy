...
public class PushAgent extends AbstractAgent {
            ...
            for (String strPort : graphitePorts.split(",")) {
                if (strPort.trim().length() > 0) {
                    GraphiteFormatter formatter =
                        new GraphiteFormatter(graphiteFormat, graphiteDelimiters);
                ...
                if (strPort.trim().length() > 0) {
                    try {
                        ...
                        JettyHttpContainerFactory
                            .createServer(new URI("http://localhost:" + strPort + "/"),
                                new ResourceConfig(JacksonFeature.class).
                                    register(
                                        new JsonMetricsEndpoint(agentAPI, agentId, port, hostname,
                                            prefix, pushLogLevel, pushValidationLevel,
                                            pushFlushInterval, pushBlockedSamples)), true);
                        logger.info("listening on port: " + strPort + " for HTTP JSON metrics");
                    } catch (URISyntaxException e) {
                        throw new RuntimeException(
                            "Unable to bind to: " + strPort + " for HTTP JSON metrics", e);
                    }
                }
            }
        }
    ...
    protected void startOpenTsdbListener(String strPort) {
        ...
        ChannelHandler graphiteHandler =
            new ChannelStringHandler(new OpenTSDBDecoder("unknown", customSourceTags), agentAPI,
                agentId, port, prefix, pushLogLevel, pushValidationLevel, pushFlushInterval,
                pushBlockedSamples, null, opentsdbWhitelistRegex, opentsdbBlacklistRegex);
    ...
    protected void startGraphiteListener(String strPort,
        ...
        // Set up a custom graphite handler, with no formatter
        ChannelHandler graphiteHandler =
            new ChannelStringHandler(new GraphiteDecoder("unknown", customSourceTags), agentAPI,
                agentId, port, prefix, pushLogLevel, pushValidationLevel, pushFlushInterval,
    ...
    protected void processConfiguration(AgentConfiguration config) {
        try {
            ...
            if (config.getCollectorSetsRetryBackoff() != null && config
