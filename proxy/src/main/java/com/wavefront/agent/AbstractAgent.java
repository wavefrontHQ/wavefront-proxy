...
public abstract class AbstractAgent {
    ...
    @Parameter(names = {"--retryThreads"},
               description = "Number of threads retrying failed transmissions. Defaults to "
                   ...
                   + "increasing the number of retry threads effectively governs the maximum "
                   + "amount of space the agent will use to buffer points locally")
    ...
    @Parameter(names = {"--pushFlushMaxPoints"},
               description = "Maximum allowed points in a single push flush. Defaults"
                   + " to 50,000")
    ...
    @Parameter(names = {"--pushListenerPorts"},
               description = "Comma-separated list of ports to listen on. Defaults to "
                   + GRAPHITE_LISTENING_PORT + ".")
    protected String pushListenerPorts = "" + GRAPHITE_LISTENING_PORT;
    @Parameter(names = {"--graphitePorts"},
               description = "Comma-separated list of ports to listen on for graphite "
    ...
    @Parameter(names = {"--graphiteDelimiters"},
               description = "Concatenated delimiters that should be replaced in the "
    ...
    @Parameter(names = {"--graphiteWhitelistRegex"},
               description = "(DEPRECATED for whitelistRegex)", hidden = true)
    protected String graphiteWhitelistRegex;
    @Parameter(names = {"--graphiteBlacklistRegex"},
               description = "(DEPRECATED for blacklistRegex)", hidden = true)
    ...
    @Parameter(names = {"--opentsdbWhitelistRegex"},
               description = "Regex pattern (java.util.regex) that opentsdb input lines must "
                   + "match to be accepted")
    ...
    @Parameter(names = {"--retryBackoffBaseSeconds"},
               description = "For exponential backoff when retry threads are throttled, the base "
                   + "(a in a^b) in seconds.  Default 2.0")
    protected double retryBackoffBaseSeconds = 2.0;
    @Parameter(names = {"--customSourceTags"},
               description = "Comma separated list of point tag keys that should be treated as "
                   + "the source in Wavefront in the absence of a tag named source or host")
    ...
    @Parameter(names = {"-c", "--config"},
               description = "Local configuration file to use (overrides using the server to "
    ...
    @Parameter(names = {"--purgeBuffer"},
               description = "Whether to purge the retry buffer on start-up. Defaults to "
                   + "false.")
    ...
    private void loadListenerConfigurationFile() throws IOException {
        ...
        if (pushConfigFile != null) {
            ...
            try {
                ...
                pushFlushInterval = Integer.parseInt(
                    prop.getProperty("pushFlushInterval", String.valueOf(pushFlushInterval)));
                pushFlushMaxPoints = Integer.parseInt(
                    prop.getProperty("pushFlushMaxPoints", String.valueOf(pushFlushMaxPoints)));
                pushBlockedSamples = Integer.parseInt(
                    prop.getProperty("pushBlockedSamples", String.valueOf(pushBlockedSamples)));
                ...
                graphiteWhitelistRegex =
                    prop.getProperty("graphiteWhitelistRegex", graphiteWhitelistRegex);
                graphiteBlacklistRegex =
                    prop.getProperty("graphiteBlacklistRegex", graphiteBlacklistRegex);
                ...
                opentsdbWhitelistRegex =
                    prop.getProperty("opentsdbWhitelistRegex", opentsdbWhitelistRegex);
                opentsdbBlacklistRegex =
                    prop.getProperty("opentsdbBlacklistRegex", opentsdbBlacklistRegex);
                splitPushWhenRateLimited = Boolean.parseBoolean(
                    prop.getProperty("splitPushWhenRateLimited",
                        String.valueOf(splitPushWhenRateLimited)));
                retryBackoffBaseSeconds = Double.parseDouble(
                    prop.getProperty("retryBackoffBaseSeconds",
                        String.valueOf(retryBackoffBaseSeconds)));
                customSourceTagsProperty =
                    prop.getProperty("customSourceTags", customSourceTagsProperty);
       ...
       * ------------------------------------------------------------------------------------ */
            ...
            for (String tag : tags) {
                ...
                if (!customSourceTags.contains(tag)) {
                    ...
                    logger.warning("Custom source tag: " + tag
                        + " was repeated. Check the customSourceTags property in wavefront.conf");
                }
            }
            ...
            if (configFile != null) {
                ...
                if (url.getPath().endsWith("/api/")) {
                    String configurationLogMessage =
                        "TO CONFIGURE THIS PROXY AGENT, USE THIS KEY: " + agentId;
        ...
        try {
            ...
            try {
                ...
                if (buffer != null) {
                    ...
                    bufferSpaceLeft.set(
                        Math.min((long) Integer.MAX_VALUE * retryThreads, buffer.getUsableSpace()));
                }
            } catch (Throwable t) {
                logger.warning("cannot compute remaining space in buffer file partition: " + t);
            }

            JsonNode agentMetrics = JsonMetricsGenerator
                .generateJsonMetrics(Metrics.defaultRegistry(), true, true, true);
            ...
            logger.warning(
                "cannot fetch proxy agent configuration from remote server: " + Throwables
                    .getRootCause(ex));
            return null;
        }
