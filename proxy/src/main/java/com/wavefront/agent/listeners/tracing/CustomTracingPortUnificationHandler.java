...
public class CustomTracingPortUnificationHandler extends TracePortUnificationHandler {
  ...
  public CustomTracingPortUnificationHandler(
    ...
    this.proxyLevelApplicationName =
        StringUtils.isBlank(customTracingApplicationName)
            ? "defaultApp"
            : customTracingApplicationName;
    this.proxyLevelServiceName =
        StringUtils.isBlank(customTracingServiceName) ? "defaultService" : customTracingServiceName;
  }
    ...
    if (wfInternalReporter != null) {
      ...
      discoveredHeartbeatMetrics.add(
          reportWavefrontGeneratedData(
              ...
              object.getSource(),
              componentTagValue,
              Boolean.parseBoolean(isError),
              millisToMicros(object.getDuration()),
              traceDerivedCustomTagKeys,
              spanTags,
              true));
      try {
