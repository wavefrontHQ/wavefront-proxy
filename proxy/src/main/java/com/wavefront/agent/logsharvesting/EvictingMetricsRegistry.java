...
public class EvictingMetricsRegistry {
  ...
  EvictingMetricsRegistry(
      ...
      long expiryMillis,
      boolean wavefrontHistograms,
      boolean useDeltaCounters,
    ...
    this.metricCache =
        Caffeine.newBuilder()
            ...
            .build();
    this.metricNamesForMetricMatchers =
        Caffeine.newBuilder().build((metricMatcher) -> Sets.newHashSet());
  }
  ...
  public Counter getCounter(MetricName metricName, MetricMatcher metricMatcher) {
    if (useDeltaCounters) {
      ...
      return put(
          newMetricName, metricMatcher, key -> DeltaCounter.get(metricsRegistry, newMetricName));
    } else {
  ...
  private <M extends Metric> M put(
      MetricName metricName, MetricMatcher metricMatcher, Function<MetricName, M> getter) {
