 ...
 */
public final class DebuggableQueueFile implements Closeable, Iterable<byte[]> {
  private static final Logger LOGGER =
      Logger.getLogger(DebuggableQueueFile.class.getCanonicalName());
  ...
  DebuggableQueueFile(String name, File file, RandomAccessFile raf, boolean zero,
                      boolean forceLegacy) throws IOException {
    this.name = name;
    this.file = file;
    this.raf = raf;
    this.zero = zero;
  ...
  private void expandIfNecessary(long dataLength) throws IOException {
    ...
    LOGGER.info("Resizing file " + name + " from " + previous + " to " + newLength);
    ...
    LOGGER.info("Resizing file " + name + " to " + newLength + " DONE (" + elapsedMillis + "ms) ");
  }
  ...
  private final class ElementIterator implements Iterator<byte[]> {
    ...
    @Override public byte[] next() {
      ...
      try {
        ...
        throw DebuggableQueueFile.<Error>getSneakyThrowable(e);
      }
    }
    ...
    @Override public void remove() {
      ...
      try {
        DebuggableQueueFile.this.remove();
      } catch (IOException e) {
        throw DebuggableQueueFile.<Error>getSneakyThrowable(e);
      }
  ...
  /** Fluent API for creating {@link DebuggableQueueFile} instances. */
  public static final class Builder {
     ...
     * file at a time.
     */
    public DebuggableQueueFile build() throws IOException {
      RandomAccessFile raf = initializeFromFile(file, forceLegacy);
      DebuggableQueueFile qf = null;
      try {
        qf = new DebuggableQueueFile(file.getPath(), file, raf, zero, forceLegacy);
        return qf;
      } finally {
        if (qf == null) {
          raf.close();
